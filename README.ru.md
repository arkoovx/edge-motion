...
## Режим двойного тапа (Double-tap hold)

Начиная с версии 1.3.3, вы можете включить активацию только по двойному тапу:
- `--double-tap-hold`: включает режим.
- `--double-tap-window-min <мс>`: минимальное время между тапами (по умолчанию 50).
- `--double-tap-window-max <мс>`: максимальное время между тапами (по умолчанию 290).

Это предотвращает случайные срабатывания при обычном ведении пальца по краю.
...
- `--threshold-bottom 0.0..0.5` — порог нижней грани; по умолчанию `0.0` (нижняя грань отключена, чтобы не мешать кликам).
- `--button-zone 0.0..0.4` — нижняя зона (область физических кнопок/края), где edge-активация дополнительно отключена.
- `--button-cooldown-ms` — короткая пауза после клика, чтобы не было случайной прокрутки при нажатии.
...
### 4) Курсор «конфликтует» с мышью после ребута

Обновите бинарник до свежей версии: теперь автовыбор устройства жёстче отфильтровывает "не-тачпад" устройства и лучше переживает одновременное подключение внешней мыши.
